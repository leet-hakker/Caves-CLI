...
class Map(object):
	...
	def get_map(self, position):
		...
		if position[0] >= 0:
			...
			nearest_x = int(round(position[0] / float(self.shape[0])) * self.shape[0]) - self.shape[0]
		if position[1] >= 0:
			nearest_y = int(round(position[1] / float(self.shape[1])) * self.shape[1])
		else:
			nearest_y = int(round(position[1] / float(self.shape[1])) * self.shape[1]) - self.shape[1]
